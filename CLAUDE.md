...
- **`rut.py`** — Core `Rut` class. Two distinct entry points: **`Rut(s)`** is the *builder* — normalizes input (strips whitespace, alt-hyphens, leading zeros) then raises on invalid; accepts bare base and auto-computes DV. **`Rut.parse(s, modo=)`** is the *classifier* — returns a `ValidacionResultado` with `estado` without raising; respects the `modo` (ESTRICTO rejects internal whitespace, FLEXIBLE accepts with a NORMALIZACION_* warning). Also: `normalizar()`, `enmascarar()`, `sugerir()`, `mejorar()`. LRU-cached factory `obtener_rut()`.
